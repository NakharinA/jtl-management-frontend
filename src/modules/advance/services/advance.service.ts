...
export const advanceService = {
  getAll: async (startDate?: string, endDate?: string): Promise<AdvanceRecord[]> => {
  ...
  getByDateRange: async (startDate: string, endDate: string): Promise<AdvanceRecord[]> => {
    return advanceService.getAll(startDate, endDate);
  },

  getByEmployeeAndMonth: async (employeeId: string, year: number, month: number): Promise<AdvanceRecord[]> => {
